...
describe("Better Bounty", function () {
  ...
  describe("When expiring a bounty", () => {
    it("should revert if bounty hasnt passed the deadline", async () => {
      await bountyContract.fundBounty(
        bounty.id,
        bounty.deadline,
        bounty.startedAt,
      ...
      await expect(bountyContract.expireBounty(bounty.id)).to.be.revertedWith(
        "BetterBounty__BountyHasNotExpiredYet()"
      );
    });
